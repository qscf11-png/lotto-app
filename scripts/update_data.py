...
def update_file(big_draws, super_draws):
    # 抓不到資料時必須以非零狀態碼結束，讓 GitHub Actions 顯示失敗並寄送通知信
    # （否則爬蟲默默壞掉，workflow 永遠綠燈，沒人會發現資料停更）
    if not big_draws or not super_draws:
        print("ERROR: Insufficient data to update. "
              f"(Big Lotto: {len(big_draws)}, Super Lotto: {len(super_draws)})")
        sys.exit(1)

    # 期數異常偏少也視為資料來源異常（正常應接近 100 期）
    if len(big_draws) < 50 or len(super_draws) < 50:
        print(f"ERROR: Suspiciously few draws fetched. "
              f"(Big Lotto: {len(big_draws)}, Super Lotto: {len(super_draws)})")
        sys.exit(1)
